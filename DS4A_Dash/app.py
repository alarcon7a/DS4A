...
engine = create_engine('postgresql://nps_demo_user:pass@nps-demo-instance.cpc1qw95qr14.us-east-2.rds.amazonaws.com/nps_demo_db')
df = pd.read_sql("SELECT * from trades", engine.connect(), parse_dates=('Entry time'))
df['Margin'] = df['Margin'].astype('int64')
df['month'] = df['Entry time'].apply(lambda x: str(x)[0:7])
...
def filter_df(exchange, leverage, start_date, end_date):
    ...
    dff = dff[dff['Margin']==int(leverage)]
    dff = dff[dff['Entry time']>=start_date]
    dff = dff[dff['Entry time']<=end_date]
...
def update_picker(dato):
    a = df[df['Exchange']==dato]
    return a['Entry time'].min().to_pydatetime(), a['Entry time'].max().to_pydatetime()    
...
def calc_returns_over_month(dff):
    ...
    for name, group in dff.groupby('month'):
...
@app.callback(
    [
        ...
        dash.dependencies.Output('strat-returns-div', 'children'),
...
def pnl_def(dff):
    ...
    for name, group in dff.groupby('Trade type'):
        data.append(
            go.Bar(y=group['Pnl (incl fees)'], x=group['Entry time'], name=name, orientation='v')
        )
    return data
...
def actualizarpnl(exchange, leverage, start_date, end_date):

    dff = filter_df(exchange, int(leverage), start_date, end_date)
...
def UpdateDailyBTC(exchange, leverage, start_date, end_date):

    dff = filter_df(exchange, int(leverage), start_date, end_date)
    data = []
    data.append(go.Scatter(x=dff['Entry time'], y=dff['BTC Price'] ))
    return  {
        'data': data,
        'layout': {
            'title': {
...
def UpdateBalance(exchange, leverage, start_date, end_date):

    dff = filter_df( exchange, int(leverage), start_date, end_date)
    data = []
    data.append(go.Scatter(x=dff['Entry time'], y=(dff['Exit balance'] - dff['Profit']) ))
    return  {
        'data': data,
        'layout': {
            'title': {
